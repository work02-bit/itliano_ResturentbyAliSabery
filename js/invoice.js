    ...
    <div class="invoice-card">
      ...
      <div class="invoice-body">
        <div class="invoice-meta">
          ...
          ${order.customer.phone2 ? `<div><span>رقم إضافي</span><strong dir="ltr">${order.customer.phone2}</strong></div>` : ""}
          ...
          ${order.customer.notes ? `<div><span>ملاحظات عامة</span><strong>${order.customer.notes}</strong></div>` : ""}
        </div>
        ...
        <table class="invoice-table">
          ...
          <tbody>
            ${order.items.map(i => `
              <tr>
                <td>
                  ${i.name}${i.size ? ` (${SIZE_LABELS[i.size] || i.size})` : ""}
                  ${i.note ? `<div class="invoice-item-note">ملاحظة: ${i.note}</div>` : ""}
                </td>
                <td>${i.qty}</td>
