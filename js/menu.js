...
function productCard(cat, item, idx){
  ...
  const mediaStyle = item.img ? ` style="background-image:url('${item.img}')"` : "";
  const mediaClass = item.img ? " has-photo" : "";

  return `
    <article class="product-card" data-uid="${uid}" data-name="${item.n}">
      ${item.tag ? `<span class="p-tag">${item.tag}</span>` : ""}
      <div class="product-media${mediaClass}"${mediaStyle}>
        <span>${iconSvg(cat.icon)}</span>
      </div>
...
document.addEventListener("click", (e) => {
  ...
  if (sizeBtn){
    ...
    card.querySelector("[data-price-display]").textContent = newPrice;
